...
class TestVBQC:
    ...
    def test_noise_protocols(self, fx_rng:np.random.Generator):
        import csv
        import re
        ...
        states = [BasicStates.PLUS for _ in pattern.input_nodes]
        secrets = Secrets(a=True, r=True, theta=True)
        # Threshold should be defined as 1/(2*k) where $k$ is the number of types of test rounds, depending on the protocol
        parameters = TrappifiedSchemeParameters(comp_rounds=100, test_rounds=100, threshold=None)
        for protocol in [FK12, Dummyless, RandomTraps]:
            print("PROTOCOL: ", protocol)
            pattern = load_pattern_from_circuit(circuit_label=random_circuit_label)
            client = Client(pattern=pattern, input_state=states, secrets=secrets, protocol_cls=protocol, parameters=parameters)
            print("Client created")
            client.trappifiedScheme.params.threshold = client.trappifiedScheme.params.test_rounds/(2*len(client.test_runs))

            depol_param_sweep = [1e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1]
            depol = {
                f"depolarising-{p}": DepolarisingNoiseModel(entanglement_error_prob=p) for p in depol_param_sweep
            }
            ...
            for noise_model_label, noise_model in combined_noise_models.items():
                canvas = client.sample_canvas()
                outcomes = client.delegate_canvas(canvas=canvas, backend_cls=DensityMatrixBackend, noise_model=noise_model)
                decision, result, result_analysis = client.analyze_outcomes(canvas=canvas, outcomes=outcomes)
                correct_value = str(find_correct_value(random_circuit_label))

                # Determine the type and param from the noise_model_label
                ...
                results_summary.append({
                    ...
                    "output2[0]": result_bit,
                    "output3": correct_value,
                    "match": match_flag,
                    "computation_outcomes_count": result_analysis.computation_outcomes_count
                })

        # After all protocols, save to CSV
        with open("noise_protocol_analysis.csv", "w", newline="") as csvfile:
            fieldnames = [
                "protocol", "noise_model", "parameter", "n_failed_test_rounds",
                "output2[0]", "output3", "match", "computation_outcomes_count"
            ]
        ...
        print("Results saved to noise_protocol_analysis.csv")
